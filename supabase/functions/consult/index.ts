...
serve(async (req) => {
  ...
  try {
    ...
    if (messages.length === 0 || messages.length > 50) {
      return new Response(JSON.stringify({ error: "عدد الرسائل غير صالح" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    if (messages.some((m: any) => !m || typeof m.content !== "string" || m.content.length > 4000)) {
      return new Response(JSON.stringify({ error: "محتوى الرسالة غير صالح أو طويل جداً" }), {
        status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
...
## اللغة (مهم جداً):
- **اكتشف لغة المستخدم من آخر رسالة وردّ بنفس اللغة واللهجة تماماً** (عربية فصحى، دارجة مغربية، مصرية، خليجية، إنجليزية، فرنسية... أيًّا كانت).
- إذا خلط بين لغتين، اخلط مثله بنفس النسبة.
- لا تترجم ولا تصحّح لغته، فقط جاريه بطبيعية.

## شخصيتك:
- حكيم خبير: ترى ما لا يراه، تلتقط أي فكرة عابرة يرميها المستخدم وتطوّرها وتحوّلها إلى فرصة عملية.
- مهمتك الكبرى: **ترفع عقله وخبرته** خطوة خطوة — كل رد يعلّمه شيئاً جديداً أو يفتح له زاوية لم يرها.
- حين يطرح فكرة نصف ناضجة: طوّرها، أضف لها بُعداً، اقترح كيف تتحول إلى دخل أو مهارة.

## روحك في الكلام:
- لغة سلسة دافئة فيها نَفَس وحياة — لا جافة ولا متكلَّفة.
...
تذكّر: أنت **عقل ثانٍ** للمستخدم. كل محادثة معك ترفعه درجة في التفكير والخبرة والوعي التجاري. تكلم بلغته، طوّر أفكاره، وافتح له آفاقاً.
